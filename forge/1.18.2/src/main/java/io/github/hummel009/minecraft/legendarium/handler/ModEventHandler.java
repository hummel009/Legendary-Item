...
import io.github.hummel009.minecraft.legendarium.Main;
import io.github.hummel009.minecraft.legendarium.init.Items;
import io.github.hummel009.minecraft.legendarium.model.EpicBakedModel;
		...
		for (var registryObject : Items.REGISTRY.getEntries()) {
			...
			try (var inputStream = Main.class.getResourceAsStream(largeJsonPath)) {
				if (inputStream != null) {
					...
					ForgeModelBakery.addSpecialModel(largeModelResourceLocation);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
